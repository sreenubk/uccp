app.controller('selectCohortCtrl', ['$scope', '$rootScope','$uibModal', '$http',  '$location','myVars',function($scope, $rootScope,$uibModal, $http,$location,myVars) {
		    	   ...
		    	   if(response.data.message){
			        	...
			        	// console.log("cohort"+$rootScope.addCohort.cohortvo.name);
			        	 ...
			        	 //$location.path('/viewControlGroup');
				        	...
				        	$http.post(myVars.site_prefix +"/TransactionController/listAllRegRequestWCohort/", regReqdata, config) 
						  		   .then(
						  		       function(response){
						  		    	 $scope.sucessMsg="";
    ...
    /** End of view Cohort **/
	 ...
	 $scope.selAllCohort = function(){
	    	...
	    	$http.post(myVars.site_prefix +"/TransactionController/listAllRegRequest/", null, config)
			   .then(
			       function(response){
			    	   if(response.data.message){
